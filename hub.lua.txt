...
-- Fix salto infinito para Delta movil
local ultimoEstadoSalto = false
RunService.Heartbeat:Connect(function()
    if getgenv().InfiniteJump then
        local char = LocalPlayer.Character
        local hum = char and char:FindFirstChildOfClass("Humanoid")
        if hum then
            local cayendo = hum:GetState() == Enum.HumanoidStateType.Freefall
            if cayendo and not ultimoEstadoSalto then
                ultimoEstadoSalto = true
                hum:ChangeState(Enum.HumanoidStateType.Jumping)
            elseif not cayendo then
                ultimoEstadoSalto = false
            end
        end
    else
        ultimoEstadoSalto = false
